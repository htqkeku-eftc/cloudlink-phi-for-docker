	...
	if client.AmIAHost() {
		...
		if settings != nil {
			...
			if settings.AllowHostReclaim {
				...
				} else { // If server-based host reclaim is enabled, transfer ownership to the next available peer
			...
			} else { // if reclaim is disabled, tell all peers that the lobby is closing and they need to leave, then destroy all connections and keys.
